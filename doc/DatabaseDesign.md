...
The reason why I add index to (Passenger.PassengerFirstName, Passenger.PassengerLastName) is that this combination of attributes is used in SELECT clause, so that adding an index can improve the query performance. Based on the screenshot, before adding the index, the actual time to get first row is 0.579 ms and the actual time to get all rows is 1.09 ms. After adding the index, the actual time to get first row is 0.359 ms and the actual time to get all rows is 0.654 ms. Thus, the query performance is improved.
...
The reason why I add index to (Ticket.PassengerId, Ticket.FlightNumber) is that this combination of attributes is used in JOIN clause and SELECT clause of the subquery, so that adding an index can improve the query performance. Based on the screenshot, before adding the index, the actual time to get first row is 0.579 ms and the actual time to get all rows is 1.09 ms. After adding the index, the actual time to get first row is 0.362 ms and the actual time to get all rows is 0.635 seconds. Thus, the query performance is improved.
...
The reason why I add index to Flight.FlightNumber is that this attribute is used in JOIN clause, so that adding an index can improve the query performance. Based on the screenshot, before adding the index, the actual time to get first row is 0.579 ms and the actual time to get all rows is 1.09 ms. After adding the index, the actual time to get first row is 0.373 ms and the actual time to get all rows is 0.689 ms. Thus, the query performance is improved.
...
The reason why I add index to Airport.AirportName is that this attribute is in the SELECT clause, so that adding an index can improve the query performance. Before adding the index, the actual time to get the first row of the result is 7.89 ms and the actual time to get all rows is 7.92 ms. After adding the index, the actual time to get first row is 4.42 ms and the actual time to get all rows is 4.44 ms. The query performance is improved.
